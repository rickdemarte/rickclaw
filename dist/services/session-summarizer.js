...
exports.SessionSummarizer = void 0;
...
class SessionSummarizer {
    ...
    shouldSummarize(currentUserMsgCount, lastSummarizedCount) {
        return (currentUserMsgCount - lastSummarizedCount) >= SUMMARY_INTERVAL;
    }
    /** Gera ou atualiza a ata da sessao */
    async summarize(previousSummary, previousKeywords, recentMessages) {
        ...
        try {
            ...
            logger_1.logger.error(`[SessionSummarizer] Erro na sumarizacao: ${err.message}`);
            return {
                summary: previousSummary || '',
                keywords: previousKeywords || '{}'
            };
    ...
    async condense(currentSummary, currentKeywords) {
        if (currentSummary.length <= SUMMARY_MAX_CHARS) {
            return { summary: currentSummary, keywords: currentKeywords };
        }
        logger_1.logger.info(`[SessionSummarizer] Ata com ${currentSummary.length} chars, condensando...`);
...
(JSON atualizado de conceitos-chave)`;
        try {
            ...
            logger_1.logger.error(`[SessionSummarizer] Erro na condensacao: ${err.message}`);
            return { summary: currentSummary, keywords: currentKeywords };
        }
    }
    buildSummarizationPrompt(previousSummary, previousKeywords) {
...
(JSON de conceitos-chave)`;
    ...
    parseResponse(raw, fallbackSummary, fallbackKeywords) {
...
exports.SessionSummarizer = SessionSummarizer;
